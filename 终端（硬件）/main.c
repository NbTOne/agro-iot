...
* ÊµÑéËµÃ÷       : LCD1602ÏÔÊ¾ÎÂ¶ÈÖµ£¬²¢ÇÒ½«ÎÂ¶ÈÖµÍ¨¹ı´®¿Ú·¢ËÍµ½µçÄÔÉÏ¡£
* Á¬½Ó·½Ê½       : ¼ûÁ¬½ÓÍ¼
* ×¢    Òâ		 : 
*******************************************************************************/
...
{
  	unsigned char symbol,i, datas[] = {0, 0, 0, 0, 0}; //¶¨ÒåÊı×é
	float tp;  
	if(temp< 0)				    //µ±ÎÂ¶ÈÖµÎª¸ºÊı
  	{
	  	LcdWriteCom(0x80);		//Ğ´µØÖ· 80±íÊ¾³õÊ¼µØÖ·
		symbol='-';               //½«½ÓÊÕµ½µÄÊı¾İ·ÅÈëµ½·¢ËÍ¼Ä´æÆ÷
...
void UsartConfiguration()
{
	...
	PCON=0X80;			//²¨ÌØÂÊ¼Ó±¶
	TH1=0XFA;				//¼ÆÊıÆ÷³õÊ¼ÖµÉèÖÃ£¬×¢Òâ²¨ÌØÂÊÊÇ9600µÄ
	TL1=0XFA;
//	ES=1;						//´ò¿ª½ÓÊÕÖĞ¶Ï
//	EA=1;						//´ò¿ª×ÜÖĞ¶Ï
	TR1=1;					//´ò¿ª¼ÆÊıÆ÷
